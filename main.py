...
def ending_insert(linestart,endstatement):
    ...
    start_spaces = spaces
    try:
        while code_indent(current_line) > spaces or "else" in python_processing[current_line]:
            current_line+=1
            if len(python_processing) == current_line: #Handles situations where the indent runs to end of file
                break
    except:
...
MyPython = open("MyPythonFile.txt")
...
for i in range(len(python_processing)):
    if "=" in python_processing[i]:
        if "==" in python_processing[i]:
            ...
            python_processing[i] = python_processing[i].replace(" = ","<-") #Can't use Unicode arrows on windows.
...
while i < len(python_processing): # Need to use while, because list gets bigger!
    ...
    if "elif" in python_processing[i]:
        python_processing[i] = python_processing[i]+"  //You will need to change this to CASE OF"
    ...
    if "for " in python_processing[i]:
        ...
        if "range" in python_processing[i]:
            if re.search("[0-9]{1},[0-9]{1,3}",python_processing[i]):
                ...
                python_processing[i] = python_processing[i].replace('in range(','0 TO ')
                python_processing[i] = python_processing[i].replace('):','')
                ending_insert(i,"NEXT")
        else:
            python_processing[i] = python_processing[i]+"  //Pseudocode can't handle this"
...
for i in range(len(python_processing)):
    if "break" in python_processing[i]:
            python_processing[i] = python_processing[i]+"  //This might be better as a repeat loop"
